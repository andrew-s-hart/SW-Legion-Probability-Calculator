...
namespace LegionCalculator
{
    public partial class Form1 : Form
    {
        ...
        private void btnReset_Click(object sender, EventArgs e)
        {
            ...
            updnRedAttack.Value = 0;
            updnBlackAttack.Value = 0;
            updnWhiteAttack.Value = 0;

            rbtnRedDefence.Checked = false;
            rbtnWhiteDefence.Checked = false;
            rbtnNoDefence.Checked = true;
            //Attack Keywords

            updnImpact.Value = 0;
            updnPierce.Value = 0;
            updnPrecise.Value = 0; //Aim token allows for 2 rerolls automattically.
            updnRam.Value = 0;
            updnMarksman.Value = 0;
            //Surges

            rbtnSurgeToCrit.Checked = false;
            rbtnSurgeToHits.Checked = false;
            rbtnSurgesNone.Checked = true;

            ckbxSurgeToBlock.Checked = false;
            updnCritical.Value = 0;
            updnAttackerSurgeTokens.Value = 0;
            updnDefenceSurgeTokens.Value = 0;
            //Tokens

            UpDnAimToken.Value = 0;
            UpDnDodgeToken.Value = 0;
            updnShield.Value = 0;
            updnObservation.Value = 0;
            //Cover

            rbtnHeavyCover.Checked = false;
            rbtnLightCover.Checked = false;
            rbtnNoCover.Checked = true;
            ckbxArmour.Checked = false;
            UpDnArmourX.Value = 0;
            //Deflect

            ckbxDeflect.Checked = false;
            //Defence

            updnLuck.Value = 0;
            UpDnDangerSense.Value = 0;
            ckbxImpervious.Checked = false;
            ckbxOutman.Checked = false;
            ckbxForceBarrier.Checked = false;
            //System
            updnItterations.Value = 1000000;
        }
        ...
        void GetInfo()//Gets all the input data
        {
            ...
            iMarksman = (int)updnMarksman.Value;
            //Surges
            ...
            iObservationTokens = (int)updnObservation.Value;
            //Cover
            ...
            bOutman = ckbxOutman.Checked;
            bForceBarrier = ckbxForceBarrier.Checked;
            //System

            iRerolls = (2 + iPrecise);
            iItterations = (int)updnItterations.Value;

            if(iMarksman > iAimTokens)
            {
                iMarksman = iAimTokens;
            }
            iAimTokens -= iMarksman;
        }
        ...
        private void btnSimulate_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= iItterations; i++)
            {
                ...
                for(int observation_count = 0; observation_count < iObservationTokens; observation_count++)
                {
                    bool NotRerolled = true;
                    foreach (Dice AttackDie in ListOfAttackDice)
                    {
                        if (NotRerolled && AttackDie.ReadResult() == "Blank" && AttackDie.ReadColour() == "RedAttack")
                        {
                            Dice Die = AttackDie;
                            RollAndConvertAttack(ref Die, ref CriticalUsed, ref SurgeTokensUsed);
                            NotRerolled = false;
                    ...
                    foreach (Dice AttackDie in ListOfAttackDice)
                    {
                        if (NotRerolled && AttackDie.ReadResult() == "Blank" && AttackDie.ReadColour() == "BlackAttack")
                        {
                            Dice Die = AttackDie;
                            RollAndConvertAttack(ref Die, ref CriticalUsed, ref SurgeTokensUsed);
                            NotRerolled = false;
                    ...
                    foreach (Dice AttackDie in ListOfAttackDice)
                    {
                        if (NotRerolled && AttackDie.ReadResult() == "Blank" && AttackDie.ReadColour() == "WhiteAttack")
                        {
                            Dice Die = AttackDie;
                            RollAndConvertAttack(ref Die, ref CriticalUsed, ref SurgeTokensUsed);
                            NotRerolled = false;
                ...
                //Apply Dodge
                int DodgesRemaining = iDodgeTokens;
                if (bOutman)
                {
                    if(DodgesRemaining > RolledCrits)
                    {
                        DodgesRemaining -= RolledCrits;
                        RolledCrits = 0;
                    }
                    else
                    {
                        RolledCrits -= DodgesRemaining;
                        DodgesRemaining = 0;
                    }
                }
                RolledHits -= DodgesRemaining;
                //Safety Checks
                if (RolledHits < 0) { RolledHits = 0; }
                if (RolledCrits < 0) { RolledCrits = 0; }
                //Apply Shileds
                int ShieldsRemaining = iShieldTokens;
                if (ShieldsRemaining > RolledCrits)
                {
                    ShieldsRemaining -= RolledCrits;
                    RolledCrits = 0;
                }
                else
                {
                    RolledCrits -= ShieldsRemaining;
                    ShieldsRemaining = 0;
                }
                RolledHits -= ShieldsRemaining;
                //Safety Checks
                if (RolledHits < 0) { RolledHits = 0; }
                if (RolledCrits < 0) { RolledCrits = 0; }
